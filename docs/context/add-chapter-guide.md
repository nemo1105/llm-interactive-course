...
- 章节案例设计遵循 `AGENTS.md` 中的模型通信、应用自定义通信和工具结果格式原则；章节文档只记录教学意图和边界，不重复代码级 payload 结构。
- 新章节应优先承接前序章节已经建立的对象、语境和调用链路，只增加本章要讲的新机制；不得为一个相邻概念另起一套无关抽象案例。
- 左侧聊天帧只展示用户真实可见的用户消息、助手思考状态和助手回答；应用内部处理、上下文构造、工程旁白和教学提示只进入右侧真实时序、章节文字或独立辅助说明。
- 右侧时序图参与者必须是真实系统 actor、运行时、工具、外部系统或用户；role、消息、上下文、提示词、token 预算、参数、缓存规则、决策点等概念信息默认由章节文字或辅助说明讲清，不作为虚构参与者。
- 右侧主时序图只画本章主链路中真实发生的事件；反事实、错误示例和概念对照需要独立 demo 或章节文字承载，不能插入主链路 step。
- 时序图 hover/focus 展示的 payload 只能是实际接口请求、接口响应、工具入参、工具结果、通道消息或应用内部状态 JSON；不得写入章节名、步骤名、教学解释、对照说明或 `event/chapter/details/teaching_rule` 一类包装字段。
- 每章内容合同必须写清本章只讲什么、暂不讲什么、承接了前面哪些已知机制，以及学员看完当前 demo 应建立的单一核心心智模型。
...
- `payloads`：消息 hover/focus 后展示的实际传输数据；同一传输可用 variants 提供不同 API 格式，JSON payload 使用结构化值并由播放器渲染为可折叠树。
...
- payload 需要新的展示形态，例如 diff、表格、token 片段或引用来源。
