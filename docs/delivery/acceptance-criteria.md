...
- 首页 `/` 不展示章节或模块列表，不展示“已确认”“待确认”等开发态状态文案，不渲染第一章演示内容或示例数据。
- 首页 `/` 不展示固定顶部栏。
- 首页 `/` 首屏展示抽象系统剖面图，桌面视口中位于右侧并延展到首屏底部。
- 首页 `/` 使用 `@fullpage/react-fullpage` 纵向展示七条课程价值；每条价值以整屏区块呈现，并支持滚轮上下切换。
- 首页 `/` 在七条课程价值之后展示进入第一章的按钮，按钮跳转到 `/chapters/01`。
